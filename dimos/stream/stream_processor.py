...
class StreamProcessor(ABC):
    ...
    def create_stream(self, input_stream: Observable) -> Observable:
        ...
        return input_stream.pipe(
            ops.map(process_with_error_handling)
        )
...
class StreamProcessorRegistry:
    ...
    def create_processor(self, processor_type: str, name: str, config: Optional[Dict[str, Any]] = None) -> StreamProcessor:
        ...
        if processor_type not in self._processors:
            raise ValueError(f"Unknown processor type: {processor_type}. Available: {list(self._processors.keys())}")
...
def create_processor(processor_type: str, name: str, config: Optional[Dict[str, Any]] = None) -> StreamProcessor:
