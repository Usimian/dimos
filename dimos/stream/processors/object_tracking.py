...
class ObjectTrackingProcessor(StreamProcessor):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if self.camera_intrinsics is not None:
                if not isinstance(self.camera_intrinsics, (list, tuple, np.ndarray)) or len(self.camera_intrinsics) != 4:
                ...
                logger.info(f"Initialized object tracking without depth estimation (no camera intrinsics)")
    ...
    def process_frame(self, frame: Any, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if self.tracker is not None and self.tracking_bbox is not None:
            if not self.tracking_initialized:
                ...
                if roi.size > 0:
                    ...
                    if self.original_des is None:
                        logger.warning("No ORB features found in initial ROI during stream processing.")
                    else:
                    ...
                    if init_success:
                        ...
                        current_bbox_x1y1x2y2 = [
                            x_init, y_init, x_init + w_init, y_init + h_init
                        ]
                ...
                if tracker_succeeded:
                    ...
                    if reid_confirmed_this_frame:
                        ...
                        logger.debug(f"Re-ID failed ({self.reid_fail_count}/{self.reid_fail_tolerance})")
        
        # Determine final success
        if tracker_succeeded:
            if self.reid_fail_count >= self.reid_fail_tolerance:
                logger.info(f"Re-ID failed consecutively {self.reid_fail_count} times. Target lost.")
                final_success = False
            else:
                final_success = True
        else:
        ...
        if final_success and current_bbox_x1y1x2y2 is not None:
            ...
            if (self.distance_estimator is not None and 
                self.distance_estimator.estimated_object_size is not None):
