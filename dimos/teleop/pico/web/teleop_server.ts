...
// WebSocket to LCM Bridge for Pico VR Teleop
// Identical to Quest bridge — just forwards binary LCM packets from browser to UDP.
// Serves the Pico-specific HTML client.
...
Deno.serve({ port: PORT, cert, key }, async (req) => {
  ...
  if (req.headers.get("upgrade") === "websocket") {
    ...
    socket.onopen = () => console.log("Pico client connected");
    socket.onclose = () => console.log("Pico client disconnected");
...
console.log(`Pico Teleop Server: https://localhost:${PORT}`);
